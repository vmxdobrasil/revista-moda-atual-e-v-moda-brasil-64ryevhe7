...
export function mapStoryTextsToExportRecords(records: StoryText[]): ExportRecord[] {
  return records.map((r) => {
    ...
    return {
      ...
      option_2: options[1] || '',
      option_3: options[2] || '',
      chars_1: (options[0] || '').length,
      chars_2: (options[1] || '').length,
      chars_3: (options[2] || '').length,
      created: r.created || '',
...
export function exportToPDF(records: ExportRecord[], filename: string): void {
  ...
  const cards = records
    .map((r, i) => {
      const pageBreak = i > 0 && i % 20 === 0 ? ' style="page-break-before: always;"' : ''
      const opts = [r.option_1, r.option_2, r.option_3]
        .map((o, idx) =>
          o
            ? `<div class="text-option"><span class="opt-label">Opção ${idx + 1} (${idx === 0 ? r.chars_1 : idx === 1 ? r.chars_2 : r.chars_3} chars):</span> ${escapeHtml(o)}</div>`
            : '',
        )
        .join('')
      return `<div class="text-card"${pageBreak}>
        <div class="text-subject">${i + 1}. ${escapeHtml(r.subject)}</div>
        <div class="text-options">${opts}</div>
