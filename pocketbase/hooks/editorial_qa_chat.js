routerAdd(
  'POST',
  '/backend/v1/agents/editorial-qa/chat',
  (e) => {
    try {
      ...
      var conv = $ai.agent('editorial-qa').getOrCreateConversation({
        user_id: userId,
        id: body.conversation_id || null,
      })

      var iter = $ai.agent('editorial-qa').chat({
        user_id: userId,
      ...
      try {
        ...
        alRec.set('integration_name', 'editorial_qa_chat')
      ...
      if (err instanceof SkipAiError) {
        ...
        return e.json(s2, {
          error: s2 >= 500 ? 'AI temporarily unavailable' : err.message,
        })
      }
      throw err
    }
